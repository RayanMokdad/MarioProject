...
def drawlevelone(screen,mario,goombas):
    ...
    draw.rect(screen,BLACK,(mario[0]+offset+10,mario[1]+72,5,5))
    draw.rect(screen,BLACK,(mario[0]+offset+45,mario[1]+72,5,5))#goomba frame later
    ...
    if mario[direction]=="left":#MARIO START
        ...
        if mario[ONGROUND]==False:
            ...
            screen.blit(marioPics[mario[CURRENT]], (350,mario[Y]))
            #MARIO FINISH
...
#    if mario[Y]+72 >= 500:
    ...
    if mask.get_at((mario[X]+10,int(mario[Y]+75))) != WALL\
       and mask.get_at((mario[X]+45,int(mario[Y]+75))) != WALL:
        mario[Y] += mario[VY]
        mario[VY] += 0.7
    elif (mask.get_at((mario[X]+10,int(mario[Y]+75))) == WALL or mask.get_at((mario[X]+45,int(mario[Y]+75))) == WALL) and mario[2]>=0:
        mario[VY] = 0
        mario[3] = True
    else:
        mario[Y] += mario[VY]
