    ...
    while running:
        ...
        for evt in event.get():
            if evt.type == QUIT:
                return "menu"
        if mb[0]== 1:
            if leveloneRect.collidepoint(mx,my):
                return "level1"
...
def level1():
    ...
    while running:
        ...
        rec = moveMario(mario,mask,gamelock,WALL)
        if rec == "deadpage":
            return "deadpage"
...
def level2():
    ...
    mask=maskL[1]
    WALL=(14, 32, 254, 255)
    coinboxs=[[575,353],[610,353],[1540,317],[2218,335],[2255,335],[4623,283],[4690,283],[5443,319]]
...
def moveMario(mario,mask,gamelock,WALL):
    ...
    if keys[K_LEFT] and mario[X] > 375 and gamelock =="OFF":
        ...
        moveleft(mario,mask,WALL)
    elif keys[K_RIGHT] and mario[X] < 7000 and gamelock == "OFF":
        mario[direction] = "right"
        mario[still] = False
        
        moveright(mario,mask,gamelock,WALL)
    else:
        mario[still] = True
    if keys[K_SPACE] and mario[ONGROUND] == True and gamelock == "OFF":
        mario[VY] =-14
        mario[ONGROUND] = False
    hitup(mario,mask,WALL)
    rec = hitdown(mario,mask,gamelock,WALL)

    if rec == "deadpage":
        return "deadpage"


def moveleft(mario,mask,WALL):
    if mario[Y] <=0:
        mario[X] -= 7
    else:
        for i in range(7):
...
def moveright(mario,mask,gamelock,WALL):
    if mario[Y] <=0:
...
def hitdown(mario,mask,gamelock,WALL):
    if gamelock == "OFF":
        if mario[VY] == -14:
            mario[Y] += mario[VY]
            #return "level1"
